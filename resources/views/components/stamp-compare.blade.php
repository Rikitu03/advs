...
@unless ($detected)
    <div class="flex items-center gap-3 rounded-xl border border-amber-400/20 bg-amber-400/5 px-4 py-3">
        ...
        <p class="text-sm text-zinc-200">No stamp region was detected by YOLOv8, so no comparison could be run. A missing-component penalty was applied to the risk score.</p>
    </div>
@else
    <div class="flex flex-col gap-4">
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-xl border border-white/10 bg-cu-surface p-4">
                ...
                <div class="flex h-24 items-center justify-center rounded-lg bg-white/[0.03] text-cu-blue">
                    <x-stamp-mark />
                </div>
            </div>
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                ...
                <p class="text-lg font-semibold text-white">{{ $data['cosine'] }}</p>
            </div>
            <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                <p class="text-xs text-cu-muted">Similarity threshold</p>
                <p class="text-lg font-semibold text-white">≥ 0.85</p>
            </div>
        </div>
    </div>
@endunless
