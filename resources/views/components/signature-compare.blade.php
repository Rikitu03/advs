...
@php
    ...
    $queryInk = $pass ? 'text-emerald-300' : 'text-rose-300';
@endphp

@unless ($detected)
    <div class="flex items-center gap-3 rounded-xl border border-amber-400/20 bg-amber-400/5 px-4 py-3">
        <flux:icon icon="exclamation-triangle" class="size-5 shrink-0 text-amber-300" />
        <p class="text-sm text-zinc-200">No signature region was detected by YOLOv8, so no comparison could be run. A missing-component penalty was applied.</p>
    </div>
@else
    <div class="flex flex-col gap-4">
        <div class="grid gap-4 sm:grid-cols-2">
            {{-- Reference --}}
            <div class="rounded-xl border border-white/10 bg-cu-surface p-4">
                ...
                <div class="flex h-24 items-center justify-center rounded-lg bg-white/[0.03] text-zinc-300">
            ...
            <div class="rounded-xl border {{ $queryRing }} bg-cu-surface p-4">
                ...
                <div class="flex h-24 items-center justify-center rounded-lg bg-white/[0.03] {{ $queryInk }}">
                    <svg viewBox="0 0 200 70" class="h-16 w-auto" fill="none" aria-hidden="true">
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            ...
            <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                ...
                <p class="text-lg font-semibold text-white">{{ $data['distance'] }}</p>
            </div>
            <div class="rounded-lg bg-white/[0.03] px-3 py-2">
                <p class="text-xs text-cu-muted">Distance threshold</p>
                <p class="text-lg font-semibold text-white">≤ {{ $data['distance_threshold'] }}</p>
            </div>
        </div>
    </div>
@endunless
