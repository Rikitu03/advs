            ...
            <div class="mx-auto flex w-full max-w-4xl flex-col gap-6">
                ...
                <div class="cu-animate-in flex flex-col gap-2">
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h1 class="text-2xl font-semibold tracking-tight text-cu-text">Audit Entry #{{ $log->id }}</h1>
                        ...
                        <a href="{{ route('admin.audit.index') }}" wire:navigate
                           class="inline-flex items-center gap-2 rounded-xl border border-cu-border bg-cu-surface px-4 py-2.5 text-sm font-medium text-cu-muted hover:text-cu-text">
                ...
                <div class="cu-animate-in grid grid-cols-1 gap-3 sm:grid-cols-2" style="animation-delay: 60ms">
                    <div class="rounded-2xl border border-cu-border bg-cu-surface p-4">
                        ...
                        @if ($log->user)
                            <p class="mt-1 text-sm font-medium text-cu-text">{{ $log->user->name }}</p>
                        ...
                        @else
                            <p class="mt-1 text-sm font-medium text-cu-text">(deleted user)</p>
                        @endif
                    </div>
                    ...
                    <div class="rounded-2xl border border-cu-border bg-cu-surface p-4">
                        ...
                        @if ($log->entity_type)
                            <p class="mt-1 text-sm text-cu-text">
                ...
                <div class="cu-animate-in rounded-2xl border border-cu-border bg-cu-surface p-4" style="animation-delay: 120ms">
                    ...
                    <pre class="mt-3 overflow-x-auto rounded-xl border border-cu-border bg-cu-bg p-4 text-xs text-cu-text">@json($log->details, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)</pre>
                </div>
                ...
                @if (is_array($before) || is_array($after))
                    <div class="cu-animate-in rounded-2xl border border-cu-border bg-cu-surface p-4" style="animation-delay: 180ms">
                        ...
                        <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
                            <div>
                                ...
                                <pre class="mt-1 overflow-x-auto rounded-xl border border-cu-border bg-cu-bg p-3 text-xs text-rose-700 dark:text-rose-200">@json($before, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)</pre>
                            </div>
                            <div>
                                <p class="text-[10px] uppercase tracking-wider text-cu-muted">After</p>
                                <pre class="mt-1 overflow-x-auto rounded-xl border border-cu-border bg-cu-bg p-3 text-xs text-emerald-700 dark:text-emerald-200">@json($after, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES)</pre>
                            </div>
                        </div>
                    </div>
                @endif
