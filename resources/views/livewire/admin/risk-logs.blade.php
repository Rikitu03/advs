...
<x-page>
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6">
        ...
        {{-- Filter bar --}}
        <div class="cu-animate-in flex flex-col gap-3 rounded-2xl border border-white/5 bg-cu-surface p-4" style="animation-delay: 60ms">
                    ...
                    <div class="flex items-center gap-1 rounded-xl border border-white/10 bg-cu-bg p-1">
                        @foreach (['all' => 'All severity', 'high' => 'High', 'medium' => 'Medium'] as $value => $text)
                            <button type="button" wire:click="setSeverity('{{ $value }}')"
                                    class="rounded-lg px-3 py-1.5 text-sm font-medium transition {{ $severity === $value ? 'bg-cu-purple text-white' : 'text-cu-muted hover:text-white' }}">{{ $text }}</button>
                        @endforeach
                    </div>
                    <div class="flex items-center gap-1 rounded-xl border border-white/10 bg-cu-bg p-1">
                        @foreach (['all' => 'All time', '24' => '24h', '168' => '7 days', '720' => '30 days'] as $value => $text)
                            <button type="button" wire:click="setRange('{{ $value }}')"
                                    class="rounded-lg px-3 py-1.5 text-sm font-medium transition {{ $range === $value ? 'bg-cu-purple text-white' : 'text-cu-muted hover:text-white' }}">{{ $text }}</button>
                        @endforeach
                    </div>
                </div>
            </div>
            {{-- Flag-type filter (§4: text mismatch, low classification confidence, signature mismatch, stamp mismatch) --}}
            <div class="flex flex-wrap items-center gap-1 self-start rounded-xl border border-white/10 bg-cu-bg p-1">
                <button type="button" wire:click="setType('all')"
                        class="rounded-lg px-3 py-1.5 text-sm font-medium transition {{ $type === 'all' ? 'bg-cu-purple text-white' : 'text-cu-muted hover:text-white' }}">All flags</button>
                @foreach ($types as $key => $meta)
                    <button type="button" wire:click="setType('{{ $key }}')"
                            class="rounded-lg px-3 py-1.5 text-sm font-medium transition {{ $type === $key ? 'bg-cu-purple text-white' : 'text-cu-muted hover:text-white' }}">{{ $meta['label'] }}</button>
                @endforeach
            </div>
        </div>
                ...
                <table class="min-w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-white/5">
                        @foreach ($rows as $log)
                            ...
                            <tr wire:key="log-{{ $log['id'] }}" class="cu-animate-in transition hover:bg-white/[0.03]" style="animation-delay: {{ min($loop->index * 40, 320) }}ms">
                                <td class="px-5 py-4">
