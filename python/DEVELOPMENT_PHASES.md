...
| 3 | **Siamese CNN** | Signature verify (128-D) | Stage 4a | `siamese_signature.h5` + `signature_threshold.txt` | `SIGNATURE_DISTANCE_THRESHOLD` (empirical / EER) |
| 4 | **EfficientNet-B0** | Stamp/logo feature + tamper texture | Stage 4b | `efficientnet_stamp.h5` + `stamp_threshold.txt` | `STAMP_SIMILARITY_THRESHOLD = 0.85` |
...
- **Signature** (`data/training/signature_data/<vendor>/`): genuine signatures per vendor; the Siamese
  trainer synthesizes forgeries (elastic + rotation + noise) and builds pairs.
- **Stamp** (`data/training/stamp_data/{genuine,forged}`): genuine seal/logo crops + forged/photocopied
  variants.
...
## Phase 6 — Siamese CNN signature verification ❌ · (M4)
...
**Tasks:** synthesize forgeries; build genuine/forged pairs (hold out ~20% of vendors); shared
`ResNet50(pooling='avg')` → `Dense(128)` + L2-norm twin towers → Euclidean → `Dense(1, sigmoid)` (or
contrastive — document which); save `siamese_signature.h5` + `siamese_encoder.h5`; compute **EER** →
...
## Phase 7 — EfficientNet stamp/logo verifier ❌ · (M5)
...
**Tasks:** `EfficientNetB0(include_top=False, pooling='avg')` (1280-D); load `stamp_data/{genuine,forged}`
(224×224); small classifier (`stamp_classifier.pkl`) **or** cosine threshold (`stamp_threshold.txt`);
save `efficientnet_stamp.h5`. Verify issuer logic: compare query vs the issuer reference (by
`document_type`, or `+city` for LGU); no reference yet → `{"match": false, "reason": "unreferenced_logo"}`
(or `no_issuer_logo` when `issuer_scope` is null); **tamper texture check runs regardless**.
