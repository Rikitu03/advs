...
> **ADVS project note:** this app's backend is **Laravel/PHP** (see the `laravel-best-practices` skill), not Node/Express — these patterns apply only to a future Node service. For ADVS domain behavior (validation pipeline, queued jobs, risk scoring, storage, tunable parameters), see [`ADVS_System_Reference.md`](../../../ADVS_System_Reference.md) `§5`, `§8`, `§9` and the `advs-system-reference` skill.
