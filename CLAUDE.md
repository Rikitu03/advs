...
> **Phased implementation plans (concern-split) live in [`docs/phases/`](docs/phases/).** The original 10-sprint roadmap in §10 is now elaborated into three parallel, concern-scoped phase plans, each folding in the **Negofood Solution** client-interview direction (compliance lifecycle: expiration monitoring, renewal reminders, completeness checklists, OCR field extraction, personnel onboarding, resubmission, food-business document types — see [`docs/CLIENT_INTERVIEW_GAP_PLAN.md`](docs/CLIENT_INTERVIEW_GAP_PLAN.md)):
